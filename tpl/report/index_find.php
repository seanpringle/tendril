<form method="GET" class="search">
	...
	<strong>index</strong>
	<input type="text" name="index" value="<?= escape(pkg()->request('index')) ?>" placeholder="regex" />
...
foreach ($rows as $row)
{
	...
	$cells = array(
		...
		tag('td', array(
			'html' => escape(pkg()->format_csv($row['table_names'])),
		)),
		tag('td', array(
			'html' => escape(pkg()->format_csv($row['index_names'])),
		)),
	);
