...
drop event if exists tendril_global_status_log_1d;
drop event if exists tendril_purge_global_status_log_1d;
drop event if exists tendril_global_status_log_7d;
drop event if exists tendril_purge_global_status_log_7d;
drop event if exists tendril_global_status_log_5m;
drop event if exists tendril_purge_global_status_log_5m;

delimiter ;;

create event tendril_global_status_log_1d
  on schedule every 5 minute starts date(now())
  do begin

    if (get_lock('tendril_global_status_log_1d', 1) = 0) then
    ...
    do release_lock('tendril_global_status_log_1d');
  end ;;

  create event tendril_purge_global_status_log_1d
  on schedule every 5 minute starts date(now())
  do begin

    if (get_lock('tendril_purge_global_status_log_1d', 1) = 0) then
      signal sqlstate value '45000' set message_text = 'get_lock';
    end if;
  ...
  create event tendril_global_status_log_7d
  on schedule every 30 minute starts date(now())
  do begin

    if (get_lock('tendril_global_status_log_7d', 1) = 0) then
      signal sqlstate value '45000' set message_text = 'get_lock';
    end if;
    ...
    do release_lock('tendril_global_status_log_7d');
  end ;;

  create event tendril_purge_global_status_log_7d
  on schedule every 30 minute starts date(now())
  do begin

    if (get_lock('tendril_purge_global_status_log_7d', 1) = 0) then
      signal sqlstate value '45000' set message_text = 'get_lock';
    end if;
